...
// Normalize a string: lowercase, no periods, trimmed spaces
...
export default function ProfessorProfile() {
  ...
  // Find all classes this professor teaches and compute GPA
  let totalGrades = 0;
  let gradeCount = 0;
  let classList: { code: string; name: string; avggrade: number }[] = [];

  for (const cls of classesData) {
    const prof = cls.professors.find((p) => p.prof === professorSlug);
    if (prof) {
      if (typeof prof.avggrade === "number") {
  ...
  console.log("🔍 Normalized First Name (partial match):", normalizedFirst);
  if (!rmpProf) {
